...
import { STUSTLERS_REVIEWS_QUERY } from "@/sanity/lib/queries";
...
const ReviewCard: React.FC<Review> = ({ comment, role, name }) => {
  return (
    <div className="min-w-[310px] lg:min-w-[528px] h-[200px] rounded-lg p-4 lg:p-8 bg-primary-light shadow-md flex flex-col justify-between">
...
const StustlersReview: React.FC = () => {
  ...
  const fetchReviews = async () => {
    try {
      setLoading(true);
      const posts = await client.fetch(STUSTLERS_REVIEWS_QUERY);
  ...
  return (
    <section
      id="review"
      className="pb-20 pt-[86px] md:pt-[129px] px-4 container mx-auto min-h-[50vh]"
    >
      <h3 className="flex items-center gap-2 sm:gap-4 text-primary justify-center">
        {/* <IoIosCheckmarkCircleOutline /> */}
        <span>HEAR FROM OUR STUSTLERS</span>
      </h3>
...
export default StustlersReview;
