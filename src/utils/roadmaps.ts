export const roadMaps = [
  {
    title: "Frontend Web Development",
    value: "frontendWebDevelopment",
    resources: "#",
    steps: [
      {
        title: "Beginner",
        outline: [
          "HTML & CSS Basics",
          "JavaScript Fundamentals",
          "Responsive Layouts & Flexbox/Grid",
          "Version Control (Git/GitHub)",
          "Basic Website Deployment",
        ],
      },
      {
        title: "Intermediate",
        outline: [
          "DOM Manipulation",
          "APIs & Data Fetching",
          "Frontend Frameworks (React, Vue, or Angular)",
          "Routing & Navigation",
          "State Management (Redux, Zustand, or Context API)",
          "Authentication & Authorization",
        ],
      },
      {
        title: "Advanced",
        outline: [
          "Performance Optimization",
          "Progressive Web Apps (PWA)",
          "Server-Side Rendering (Next.js/Nuxt.js)",
          "Testing (Jest, Cypress)",
          "CI/CD & Deployment",
          "Web Security Best Practices",
        ],
      },
    ],
  },
  {
    title: "Backend Web Development",
    value: "backendWebDevelopment",
    resources: "#",
    steps: [
      {
        title: "Beginner",
        outline: [
          "Introduction to the Web (HTTP, Client-Server Model)",
          "Basic Programming (JavaScript, Python, or Node.js)",
          "Version Control (Git/GitHub)",
          "Databases Basics (Relational vs NoSQL)",
          "APIs Fundamentals (REST)",
        ],
      },
      {
        title: "Intermediate",
        outline: [
          "Server-Side Frameworks (Express.js, Django, Flask, etc.)",
          "Database Design & Relationships",
          "Authentication & Authorization",
          "File Uploads & Handling",
          "Caching & Performance Basics",
          "API Testing (Postman, Swagger)",
        ],
      },
      {
        title: "Advanced",
        outline: [
          "Advanced APIs (GraphQL, gRPC, WebSockets)",
          "Microservices Architecture",
          "Scalability & Load Balancing",
          "Security Best Practices (OWASP, JWT, OAuth)",
          "Containerization & Deployment (Docker, Kubernetes)",
          "CI/CD Pipelines & DevOps Basics",
          "Monitoring & Logging (Prometheus, ELK Stack)",
        ],
      },
    ],
  },
  {
    title: "UI/UX Design",
    value: "uiUxDesign",
    resources: "#",
    steps: [
      {
        title: "Beginner",
        outline: [
          "Introduction to UI vs UX Design",
          "Importance of Design in Digital Products",
          "Overview of the Design Process",
          "Design Career Paths (UI, UX, Product Designer)",
          "Color Theory Basics",
          "Typography Fundamentals",
          "Layout & Composition Principles",
          "Visual Hierarchy & Balance",
          "Accessibility Basics in Design",
        ],
      },
      {
        title: "Intermediate",
        outline: [
          "Understanding Users & User Needs",
          "User Research Methods",
          "User Personas & Empathy Maps",
          "Journey Mapping & User Flows",
          "Competitor & Market Analysis",
          "Low-Fidelity Wireframing",
          "High-Fidelity Wireframing",
          "Storyboarding User Interactions",
          "Prototyping with Figma or Adobe XD",
          "Usability Testing Basics",
          "Grid Systems & Spacing",
          "Designing Buttons, Forms & Inputs",
          "Responsive & Adaptive Design",
          "Design Systems & Style Guides",
        ],
      },
      {
        title: "Advanced",
        outline: [
          "Information Architecture",
          "Interaction Design Principles",
          "Advanced Accessibility & Inclusive Design",
          "Usability Testing & Iteration",
          "Advanced Prototyping & Micro-Interactions",
          "Design Handoff & Developer Collaboration",
          "Using Figma Dev Mode & Zeplin",
          "Collaboration Tools (Miro, FigJam, Notion)",
          "Building a Strong UI/UX Portfolio",
          "Writing Case Studies",
          "Personal Branding as a Designer",
          "Preparing for Interviews & Design Challenges",
          "Contributing to Design Communities",
          "Keeping Up with Design Trends & Tools",
          "Networking & Mentorship",
        ],
      },
    ],
  },
  {
    title: "Graphics Design",
    value: "graphicsDesign",
    resources: "#",
    steps: [
      {
        title: "Beginner",
        outline: [
          "Basic Design Tools (Canva, Photoshop, Illustrator)",
          "Color Theory",
          "Typography",
          "Composition & Layout",
        ],
      },
      {
        title: "Intermediate",
        outline: [
          "Logo Design",
          "Branding & Identity",
          "Vector Graphics & Illustration",
          "Photo Manipulation & Editing",
        ],
      },
      {
        title: "Advanced",
        outline: [
          "3D Design & Modeling",
          "Motion Graphics & Animation",
          "Packaging Design",
          "Advanced Illustration Techniques",
        ],
      },
    ],
  },
  {
    title: "Social Media Management",
    value: "socialMediaManagement",
    resources: "#",
    steps: [
      {
        title: "Beginner",
        outline: [
          "Introduction to Social Media Management",
          "Creating and Optimizing Social Media profiles",
          "Content creation fundamentals",
          "Content calendar and scheduling basics",
          "Engagement and Community management",
          "Introduction to analytics",
        ],
      },
      {
        title: "Intermediate",
        outline: [
          "Social Media Strategy Development",
          "Content Marketing & Storytelling",
          "Scheduling & Automation Tools",
          "Paid Social Media Advertising (Basics)",
          "Intermediate Analytics & Reporting",
        ],
      },
      {
        title: "Advanced",
        outline: [
          "Advanced Social Media Strategy",
          "Advanced Paid Advertising & Campaigns",
          "Branding & Positioning",
          "Data-Driven Decision Making",
          "Client & Team Management",
          "Project/Certification",
        ],
      },
    ],
  },
  {
    title: "Data Analysis / Data Science",
    value: "dataAnalysis",
    resources:
      "https://docs.google.com/spreadsheets/u/0/d/1CGoKsZjNxSER7uThQigir0-MH1_HtjvWRURNbfPxnVU/htmlview",
    steps: [
      {
        title: "Excel",
        outline: [
          "Excel for Data Analysis",
          "Excel Tutorials for Data Analysts",
          "Excel Skill Practice Exercises",
          "Excel for Analytics Projects",
          "Excel Skill Assessment Test",
        ],
      },
      {
        title: "Power BI",
        outline: [
          "Introduction to Power BI",
          "Beginner Guide to DAX",
          "DAX Foundations",
          "Power BI Virtual Internship Experience",
          "End-to-End Power BI Project",
          "Power BI Skill Assessment Test",
        ],
      },
      {
        title: "SQL",
        outline: [
          "Introduction to SQL for Data Analysis",
          "SQL Fundamentals",
          "SQL for Data Analysts and Scientists",
          "SQL Skill Practice Quizzes",
          "SQL Skill Assessment Test",
          "SQL 8-Week Project Challenge",
          "Data Analyst Portfolio Project (SQL)",
        ],
      },
      {
        title: "Tableau",
        outline: [
          "Tableau Tutorials for Beginners",
          "Free Tableau Training Videos",
          "Tableau Visualization Projects",
          "Data Visualization with Tableau Course",
        ],
      },
      {
        title: "Python",
        outline: [
          "Python for Beginners",
          "Data Analysis with Python Fundamentals",
          "Full Data Analysis with Python Course",
          "Python for Data Science",
          "Beginner Python Projects",
          "Python Data Analysis Projects",
          "Data Analyst Portfolio Project (Python)",
          "Python Skill Assessment Test",
          "Data Science Project Ideas with Python",
        ],
      },
      // {
      //   title: "Certifications",
      //   outline: ["Google Data Analytics Professional Certificate"],
      // },
    ],
  },
  {
    title: "Writing",
    value: "writing",
    resources: "#",
    steps: [
      {
        title: "Beginner",
        outline: [
          "Content Writing Basics",
          "Blogging Platforms",
          "Copywriting Essentials",
          "Grammar & Editing",
        ],
      },
      {
        title: "Intermediate",
        outline: [
          "Technical Writing",
          "SEO Writing",
          "Creative Writing Techniques",
          "Storytelling Basics",
          "Social Media Writing",
        ],
      },
      {
        title: "Advanced",
        outline: [
          "Content Strategy Development",
          "Brand Voice & Storytelling",
          "Persuasive Writing",
          "Publishing & Freelancing",
          "Advanced Editing & Proofreading",
        ],
      },
    ],
  },
  {
    title: "Virtual Assistance",
    value: "virtualAssistance",
    resources: "#",
    steps: [
      {
        title: "Beginner",
        outline: [
          "Email Management",
          "Scheduling & Calendar Management",
          "Data Entry",
          "Basic Office Tools (Word, Excel, Docs)",
        ],
      },
      {
        title: "Intermediate",
        outline: [
          "Customer Support (Chat/Email)",
          "CRM Tools",
          "Project Coordination",
          "Online Research",
        ],
      },
      {
        title: "Advanced",
        outline: [
          "Process Automation (Zapier, Make)",
          "Advanced Research",
          "Client Relationship Management",
          "Business Support & Strategy",
        ],
      },
    ],
  },
];
